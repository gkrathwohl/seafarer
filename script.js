...
fetch(url).then(function(response){
  ...
  new png.PNG({ filterType:4 }).parse( buffer, function(error, image){
    ...
    while(q.peekFront()){
      ...
      while(curr_pixel_and_distance = current_generation.pop()) {
        curr_pixel = curr_pixel_and_distance.pixel;
        curr_distance = curr_pixel_and_distance.distance;
        if(!mask[curr_pixel[0]][curr_pixel[1]]){
          continue;
        }
        if(distances[curr_pixel[0]][curr_pixel[1]] == undefined){ // || distances[curr_pixel[0]][curr_pixel[1]] > curr_distance){
        ...
        neighbors = [];
        neighbors.push({ pixel: [curr_pixel[0]+1, curr_pixel[1]], distance: curr_distance + 1})
        neighbors.push({ pixel: [curr_pixel[0], curr_pixel[1]+1], distance: curr_distance + 1})
        neighbors.push({ pixel: [curr_pixel[0]-1, curr_pixel[1]], distance: curr_distance + 1})
        neighbors.push({ pixel: [curr_pixel[0], curr_pixel[1]-1], distance: curr_distance + 1})

        // diagonals
        neighbors.push({ pixel: [curr_pixel[0]+1, curr_pixel[1]+1], distance: curr_distance + 1.4142})
        neighbors.push({ pixel: [curr_pixel[0]-1, curr_pixel[1]-1], distance: curr_distance + 1.4142})
        neighbors.push({ pixel: [curr_pixel[0]+1, curr_pixel[1]-1], distance: curr_distance + 1.4142})
        neighbors.push({ pixel: [curr_pixel[0]-1, curr_pixel[1]+1], distance: curr_distance + 1.4142})

        // knights move
        neighbors.push({ pixel: [curr_pixel[0]+1, curr_pixel[1]+2], distance: curr_distance + 2.236})
        neighbors.push({ pixel: [curr_pixel[0]+2, curr_pixel[1]+1], distance: curr_distance + 2.236})
        neighbors.push({ pixel: [curr_pixel[0]+2, curr_pixel[1]-1], distance: curr_distance + 2.236})
        neighbors.push({ pixel: [curr_pixel[0]+1, curr_pixel[1]-2], distance: curr_distance + 2.236})
        neighbors.push({ pixel: [curr_pixel[0]-1, curr_pixel[1]-2], distance: curr_distance + 2.236})
        neighbors.push({ pixel: [curr_pixel[0]-2, curr_pixel[1]-1], distance: curr_distance + 2.236})
        neighbors.push({ pixel: [curr_pixel[0]-2, curr_pixel[1]+1], distance: curr_distance + 2.236})
        neighbors.push({ pixel: [curr_pixel[0]-1, curr_pixel[1]+2], distance: curr_distance + 2.236})
        neighbors = neighbors.filter(function(n){ p = n.pixel; return p[0] <= 255 && p[0] >= 0 && p[1] <= 255 && p[1] >= 0});
    ...
    while(!skip && !(current_pixel[0] == goal[0] && current_pixel[1] == goal[1])){
      neighbors = [
        movePixel(current_pixel, 1, 0),
        movePixel(current_pixel, 0, 1),
        movePixel(current_pixel, -1, 0),
        movePixel(current_pixel, 0, -1),
        // diagonals
        movePixel(current_pixel, 1, 1),
        movePixel(current_pixel, -1, -1),
        movePixel(current_pixel, +1, -1),
        movePixel(current_pixel, -1, -1),
        // knights move
        movePixel(current_pixel, 1, 2),
        movePixel(current_pixel, 2, 1),
...
drawPath = function(){
  simplified = simplify(pointsArrayToHash(shortest_path), 1, true)

  points = simplified.map((a) => [a.y, a.x])
